...
gulp.task('webfont', function() {
  ...
  return gulp.src('dist/**/*.html')
    .pipe(fontSpider({silent: false, debug: true}))
    .pipe(notify({
...
gulp.task('font_rev', gulpSequence('webfont', 'rev'));

// build 需要插入资源指纹（MD5），html 最后执行
gulp.task('build', gulpSequence('clean', ['sass', 'css', 'browserify', 'tinypng', 'svg'], 'html', 'rev'));
